...
const FindId = () =>{
    ...
    const onClickFindId = async() => {
        const res = await MuteApi.memberFind(findInName, findInEmail, "Type_ID");
    ...
    const onClickFindPwd = async() => {
        const res = await MuteApi.memberFind(findInId, findInEmail, "Type_PWD");
    ...
    const onClickNewPwd = async() => {
        const res = await MuteApi.memberNewPwd(findInId, newPwdCon);
        if(res.data){
            setIsNewPwd(false);
            setModalOpen(true);
            setHeader("로그인");
    ...
    return (
        <>
            <p open={modalOpen} close={closeModal} header={header}>{comment}</p>
            <Link to="/" >Mute</Link>
            <div>
                        ...
                        {!isNewPwd?
                            ...
                            <>
                                ...
                                <p className="newPwd">
                                    ...
                                    <input type="password" placeholder="비밀번호" onChange={onChangePw} value={newPwd}></input>
                                </p>
                                <p className="newPwd">
                                    새 비밀번호 확인{newPwdCon.length > 0 && <span>{conPwMessage}</span>}<br/>
                                    <input type="password" placeholder="비밀번호 확인" onChange={onChangeConPw} value={newPwdCon} onKeyDown={onKeyDownNewPwd}></input>
                                </p>
